...
-- -- *************************************************************
-- -- AJOUTE CES LIGNES POUR CRÉER L'UTILISATEUR ET LUI DONNER LES PRIVILÈGES
-- -- *************************************************************

-- -- Supprime l'utilisateur s'il existe déjà pour s'assurer d'une réinitialisation propre
-- DROP USER IF EXISTS 'nextera_user'@'%';

-- -- Crée l'utilisateur 'nextera_user' avec le mot de passe 'password'
-- -- '%': permet la connexion depuis n'importe quelle adresse IP (nécessaire dans Docker)
-- -- IDENTIFIED WITH mysql_native_password: assure la compatibilité avec PyMySQL/Django
-- CREATE USER 'nextera_user'@'%' IDENTIFIED WITH mysql_native_password BY 'password';

-- -- Accorde tous les privilèges sur la base de données 'nextera' à l'utilisateur 'nextera_user'
-- GRANT ALL PRIVILEGES ON nextera.* TO 'nextera_user'@'%';

-- -- Recharge les privilèges MySQL pour qu'ils soient pris en compte immédiatement
-- FLUSH PRIVILEGES;
...
-- bdd.sql
-- Initialisation de la base de données et de l'utilisateur pour Django
